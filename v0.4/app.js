...
import { userService, eventService, memberService, roleService, crewService, forgottonPasswordService } from './innloggingService'; //importerer de
...
//The class SignIn contains a log in form.
class SignIn extends React.Component<{}> {
    ...
    //These is where the HTML elements are rendered.
    render() {
        return (
    ...
    componentDidMount() {
        ...
        // this function checks if the user typed in right username and the right password
        // if the Promise succeeds, the user will get signed in.
        // if not, an error message will occure.
        this.refs.signInButton.onclick = () => {
            userService.signIn(this.refs.signInUsername.value, this.refs.signInPassword.value).then(() => {
                ...
                document.getElementById("val").innerHTML = "Feil brukernavn eller passord! Kontoen kan også være deaktivert.";
            });
        };
    }
}
...
//the class NewMember contains a sign up form
            ...
            memberService.checkAccountDetailsFromUsernameAndEmail(this.refs.username.value, this.refs.email.value).then((result) => {
                ...
                if(result.Brukernavn==this.refs.username.value) {
                    ...
                    memberService.addMember(
                        ...
                        this.refs.adress.value).then(
                        console.log("THEN"), errorMessage.set("Bruker opprettet!"),
                        history.push('/signin')).catch((error: Error) => {
...
// The class AddEvent makes it possible to create events.
...
// The class EditEvent makes it possible to edit choosen event
