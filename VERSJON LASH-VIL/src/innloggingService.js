...
class MemberService {
  addMember(
    ...
    getOtherMembers(id) : Promise<void> {
        return new Promise((resolve, reject) => {
            connection.query('SELECT * FROM Medlemmer WHERE KontoAktiv>? AND ID!=?', [0, id], (error, result) => {
                if(error) {
    ...
    deleteMember(id) : Promise<void> {
        return new Promise((resolve, reject) => {
            connection.query("UPDATE Medlemmer SET KontoAktiv = -1 WHERE ID = ?", [id], (error, result) => {
                if(error) {
                    reject(error);
                    return;
                }
